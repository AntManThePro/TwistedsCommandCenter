...
export function QuickLister() {
const { createListing, listings, generateSquarespaceHTML, TONE_TEMPLATES } =
useListings();
const [selectedTone, setSelectedTone] = useState<‘elegant’ | ‘energetic’ | ‘mystical’ | ‘professional’>(‘elegant’);
...
<h1 className="text-4xl font-bold text-[#ff0080] mb-2">
⚡ QUICK SALE LISTER
</h1>
<p className="text-[#60efff]">
Turn inventory into shop-ready listings instantly. Professional
descriptions. Shareable formats.
</p>
</div>

```
    {copied && (
      <div className="fixed top-6 right-6 bg-[#00ff87] text-[#0a0e27] px-6 py-3 rounded font-bold animate-pulse">
        ✓ Copied to clipboard!
      </div>
    )}
        ...
        <form onSubmit={handleGenerateListing} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 bg-[#1a1a3a] border border-[#00ff87] text-[#00ff87] rounded focus:outline-none focus:shadow-lg focus:shadow-[rgba(0,255,135,0.5)] h-24 resize-none"
            />
          </div>
          ...
          <div>
            <label className="block text-[#00ff87] font-bold mb-3">
              Listing Tone
            </label>
            <div className="grid grid-cols-2 gap-3">
              {(
                ['elegant', 'energetic', 'mystical', 'professional'] as const
              ).map((tone) => (
                <button
                  key={tone}
                  type="button"
        ...
        {preview ? (
          <div>
            <div className="bg-[rgba(255,204,0,0.05)] border-2 border-[#ffcc00] rounded-lg p-6 mb-6">
              <h3 className="text-2xl font-bold text-[#ffcc00] mb-2">
                {preview.title}
              </h3>
            ...
            <button
              onClick={() =>
                handleCopy(preview.generatedDescription)
              }
            ...
            <div className="bg-[#0a0e27] border border-[#00ff87] p-4 rounded mb-4 overflow-x-auto text-xs">
              <code className="text-[#60efff]">
                {generateSquarespaceHTML(preview)}
              </code>
            </div>

            <button
              onClick={() =>
                handleCopy(generateSquarespaceHTML(preview))
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {listings.map((listing) => (
            ...
            >
              ...
              <p className="text-[#60efff] text-xs opacity-75">
                {listing.dateCreated}
              </p>
            </div>
          ))}
