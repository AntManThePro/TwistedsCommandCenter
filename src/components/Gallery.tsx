...
const Gallery = memo(function Gallery() {
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-[#0a0e27] to-[#1a0f3a] text-[#00ff87] flex flex-col">
      <div className="bg-gradient-to-r from-[#0a0e27] to-[#1a1a3a] via-[#2a0a2a] border-b-2 border-[#ff0080] p-8 flex-shrink-0 shadow-lg shadow-[rgba(255,0,128,0.3)]">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
            <div>
              <h1 className="text-5xl font-bold text-[#ff0080] mb-2">TWISTED GENIUS</h1>
            ...
            <div className="flex gap-8 text-[#60efff] text-right">
              <div>
                <div className="text-3xl font-bold text-[#00ff87]">{stats.total}</div>
                <div className="text-sm">Pieces</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-[#ffcc00]">${stats.value.toFixed(0)}</div>
      ...
      <div className="flex flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredArtwork.map((item) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      <h3 className="text-[#ffcc00] font-bold mb-2">{item.title}</h3>
                      <p className="text-[#60efff] text-sm capitalize mb-3">
                        {item.category.replace('-', ' ')}
                      </p>
                      {item.price > 0 && (
                        <p className="text-[#00ff87] text-lg font-bold">${item.price.toFixed(2)}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {selectedItem && (
          <div className="w-80 bg-[rgba(10,14,39,0.95)] border-l-2 border-[#00ff87] p-8 overflow-y-auto flex-shrink-0 shadow-lg shadow-[rgba(0,255,135,0.1)] relative">
            <button
            ...
            <h2 className="text-2xl font-bold text-[#ffcc00] mb-6 pr-8">{selectedItem.title}</h2>

            {selectedItem.price > 0 && (
              <div className="mb-6 pb-6 border-b border-[rgba(0,255,135,0.2)]">
                <div className="text-[#00ff87] text-sm uppercase font-bold mb-2">Price</div>
                <div className="text-4xl font-bold text-[#00ff87]">
            ...
            <div className="mb-6 pb-6 border-b border-[rgba(0,255,135,0.2)]">
              <div className="text-[#00ff87] text-sm uppercase font-bold mb-2">Category</div>
            ...
            {selectedItem.materials && (
              <div className="mb-6 pb-6 border-b border-[rgba(0,255,135,0.2)]">
                <div className="text-[#00ff87] text-sm uppercase font-bold mb-2">Materials</div>
            ...
            {selectedItem.dimensions && (
              <div className="mb-6 pb-6 border-b border-[rgba(0,255,135,0.2)]">
                <div className="text-[#00ff87] text-sm uppercase font-bold mb-2">Dimensions</div>
                <div className="text-[#60efff]">{selectedItem.dimensions}</div>
              </div>
            )}

            <div className="mb-8">
              <div className="text-[#00ff87] text-sm uppercase font-bold mb-2">Availability</div>
            ...
            <div className="bg-[rgba(96,239,255,0.1)] border-l-4 border-[#60efff] p-4 rounded text-sm text-[#60efff] leading-relaxed">
              Each piece from Twisted Genius is handcrafted with meticulous attention to detail.
              Inquire for shipping, custom orders, or to arrange a private viewing.
            </div>
          </div>
        )}
      </div>
