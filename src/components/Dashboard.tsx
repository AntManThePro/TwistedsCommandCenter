      ...
      <div className="bg-gradient-to-r from-[#0a0e27] to-[#1a1a3a] border-b-2 border-[#ff0080] p-6 shadow-lg shadow-[rgba(255,0,128,0.3)]">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="text-[#60efff] flex items-center gap-2">
            <span className="inline-block w-2 h-2 bg-[#00ff87] rounded-full animate-pulse" />
            System Live
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(categories).map(([cat, count]) => (
              ...
              >
                ...
                <div className="text-4xl font-bold text-[#ffcc00] mb-2">{count}</div>
...
function MetricCard({ label, value, change, colorClass }: MetricCardProps) {
  return (
    <div
      className={`bg-[rgba(10,14,39,0.9)] border-2 rounded-lg p-6 shadow-lg transition-all hover:-translate-y-1 ${colorClass}`}
    >
      <div className="text-sm uppercase font-bold opacity-80 mb-3">{label}</div>
